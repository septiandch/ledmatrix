...
//  From https://gcc.gnu.org/onlinedocs/gccint/Soft-float-lib-routines.html
...
/* Correction for acos calculation */
double acosc(double x) {
#ifdef QPFLIB_ACOS_CORRECTION
	double y;
	...
	if(x > -0.264 && x < -0.003)
	{
		if(x <= -0.258)
		{
			y = acos(x) + (-1.0 * (x + 0.263 - 0.077)); 
		}
		else if(x <= -0.240)
		{
			y = acos(x) - (x + 0.258 + 0.177);
		}
		else
		{
			y = acos(x) + (-1.0 * (x - 0.06178));
		}
	}
	else
	{
