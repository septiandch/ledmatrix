...
/* GLOBAL VARIABLES */
volatile char 		usart_sMessage[USART_DATA_SIZE];
...
void USARTx_IRQHandler(void)
{
	if(USART_GetITStatus(USARTx, USART_IT_RXNE) != RESET)
	{
		...
		if(usart_nIndex >= USART_DATA_SIZE || usart_sMessage[usart_nIndex - 1] == '\0')
		{
			usart_message_clear();
		}
...
{
	usart_nIndex = 0;
	memset(usart_sMessage, '\0', USART_DATA_SIZE);
}

void usart_message_retrieve(char *pcString)
{
	memcpy(pcString, usart_sMessage, usart_nIndex);
}
