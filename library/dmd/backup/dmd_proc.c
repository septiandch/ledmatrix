...
int16_t			wMarqueeWidth	= 0;
int16_t			wMarqueePos		= 0;
uint8_t			bMarqueePause	= 0;
...
void dmdproc_set_frame(int16_t nPosX, int16_t nPosY, int16_t width, int16_t height)
{
	stDispFrame.eActive = ENABLE;
	stDispFrame.wStartX = nPosX;
	stDispFrame.wStartY = nPosY;
	stDispFrame.wEndX   = nPosX + width;
	stDispFrame.wEndY   = nPosY + height;
}
...
uint16_t dmdproc_get_textcenter(char *textSource)
{
	uint16_t	width	= 1;
	uint16_t	x		= 0;
...
void dmdproc_marquee_pause(FunctionalState state)
{
	bMarqueePause = state;
}

uint16_t dmdproc_get_marqueepos(void)
{
	return wMarqueePos;
}

void dmdproc_clr_marqueepos(void)
{
	wMarqueePos = 0;
}

uint8_t dmdproc_draw_marquee(int16_t nPosX, int16_t nPosY, int16_t width, int16_t height, char *bStr, eMarqueeDirection direction, eColor color)
{
	int16_t		i				= 0;
	int16_t		wMarqueePosTmp	= wMarqueePos;
	
	/* Set initial Marquee params */
	if(wMarqueeWidth == 0)
	{
		if(direction == eMARQUEE_TOP_TO_BOTTOM)
		{
			wMarqueeWidth = height;
			wMarqueePos = nPosY - height - 2;
		}
		else if(direction == eMARQUEE_BOTTOM_TO_TOP)
		{
			wMarqueeWidth = height;
			wMarqueePos = nPosY + height + 2;
		}
		else
		{
			...
			{
				wMarqueeWidth += dmdproc_get_charwidth(bStr[i]);
				
				/* Plus char gap */
				wMarqueeWidth += 1;
			}
			
			if(direction == eMARQUEE_RIGHT_TO_LEFT)
			{
				wMarqueePos = nPosX + width + 2;
			}
			else if(direction == eMARQUEE_LEFT_TO_RIGHT)
			{
				wMarqueePos = nPosX - wMarqueeWidth - 2;
			}
		}
	}
	
	/* Draw String in frame */
	dmdproc_set_frame(nPosX, nPosY, width, height);
	
	if(direction == eMARQUEE_RIGHT_TO_LEFT || direction == eMARQUEE_LEFT_TO_RIGHT)
	{
		dmdproc_draw_string(wMarqueePos, nPosY, bStr, color);
	}
	else if(direction == eMARQUEE_TOP_TO_BOTTOM || direction == eMARQUEE_BOTTOM_TO_TOP)
	{
		dmdproc_draw_string(nPosX, wMarqueePos, bStr, color);
	}
	...
	{			
		case eMARQUEE_TOP_TO_BOTTOM :
			...
			if(wMarqueePos >= nPosY + wMarqueeWidth + 2)
			{
				wMarqueeWidth = 0;
				return 1;
			}
		...
		case eMARQUEE_BOTTOM_TO_TOP :
			...
			if(wMarqueePos <= (wMarqueeWidth + 2) * -1)
			{
				wMarqueeWidth = 0;
				return 1;
			}
	...
	if(bMarqueePause == ENABLE)
	{
		wMarqueePos = wMarqueePosTmp;
	}
