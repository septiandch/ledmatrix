			...
			switch(usart_sMessage[usart_nIndex - 2])
			{
				...
				case '!' :
					if(usart_nIndex - 1 == 18)
					{
						...
						_stPTime[0] = (stTime) { usart_sMessage[ 3], usart_sMessage[ 4] - 1 };
						_stPTime[1] = (stTime) { usart_sMessage[ 5], usart_sMessage[ 6] - 1 };
						_stPTime[2] = (stTime) { usart_sMessage[ 7], usart_sMessage[ 8] - 1 };
						_stPTime[3] = (stTime) { usart_sMessage[ 9], usart_sMessage[10] - 1 };
						_stPTime[4] = (stTime) { usart_sMessage[11], usart_sMessage[12] - 1 };
						_stPTime[5] = (stTime) { usart_sMessage[13], usart_sMessage[14] - 1 };
						_stPTime[6] = (stTime) { usart_sMessage[15], usart_sMessage[16] - 1 };
				...
				case '}' :
					...
					{
						if(usart_sMessage[0] == '{')
						pdisplay_SetColor(MEM_eCL_CLOCK, usart_sMessage[1] - '0');
						pdisplay_SetColor(MEM_eCL_DATE,	usart_sMessage[2] - '0');
						pdisplay_SetColor(MEM_eCL_TEXT,	usart_sMessage[3] - '0');
						pdisplay_SetColor(MEM_eCL_PTIME, usart_sMessage[4] - '0');
						pdisplay_SetColor(MEM_eCL_COUNT, usart_sMessage[5] - '0');
