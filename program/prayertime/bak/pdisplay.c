...
stDispColor	stDisplayColor			= {eCL_BLACK, eCL_BLACK, eCL_BLACK, eCL_BLACK};

void pdisplay_Init(void)
{
	stDisplayColor.clock	= pdisplay_GetColor(MEM_eCL_CLOCK);
	stDisplayColor.date		= pdisplay_GetColor(MEM_eCL_DATE);
	stDisplayColor.text		= pdisplay_GetColor(MEM_eCL_TEXT);
	stDisplayColor.ptime	= pdisplay_GetColor(MEM_eCL_PTIME);
	stDisplayColor.counter	= pdisplay_GetColor(MEM_eCL_COUNT);
...
void pdisplay_SetMode(eDisplayMode mode, char *message, uint8_t nPTimeList)
{
	...
	switch(mode)
	{
		case MODE_POWERSAVE :
			...
			dmd_draw_string(55, 4, sMessageBuff, stDisplayColor.clock);
		...
		case MODE_BIGMESSAGE :
			dmd_set_font(Arial16);
			utils_Timestamp(stRTime.hour, stRTime.minute, NONE, bColonState, &sMessageBuff);
			dmd_draw_string(0, 0, sMessageBuff, stDisplayColor.clock);
			
			pdisplay_nCounter += dmd_draw_marquee(40, 0, 120, 15, message, eMARQUEE_RIGHT_TO_LEFT, stDisplayColor.text);
			break;
		...
		case MODE_SMALLMESSAGE :
			...
			pdisplay_nCounter += dmd_draw_marquee(0, 8, DISPLAY_WIDTH * DISPLAY_ACROSS, 15, message, eMARQUEE_RIGHT_TO_LEFT, stDisplayColor.text);
						
			utils_Timestamp(stPTime[nPTimeList].hour, stPTime[nPTimeList].minute, NONE, ':', &sMessageBuff);
			dmd_draw_string(130, 0, sMessageBuff, stDisplayColor.ptime);
			
			utils_Concat("   ", sPrayerTime[nPTimeList], "", &sMessageBuff);
			dmd_draw_string(126 - (strlen(sMessageBuff) * 6), 0, sMessageBuff, stDisplayColor.ptime);
			break;
			
		case MODE_DATETIME :
			dmd_set_font(Arial16);
			utils_Timestamp(stRTime.hour, stRTime.minute, NONE, bColonState, &sMessageTmp);
			dmd_draw_string(0, 0, sMessageTmp, stDisplayColor.clock);
			...
			dmd_draw_string(dmd_get_textcenter(&sMessageBuff) + 20, 0, sMessageBuff, stDisplayColor.ptime);
			...
			dmd_draw_string(dmd_get_textcenter(&sMessageBuff) + 20, 9, sMessageBuff, stDisplayColor.date);
		...
		case MODE_PRAYERTIME :
			...
			pdisplay_nCounter += dmd_draw_marquee(0, 0, 160, 15, sMessageBuff, eMARQUEE_RIGHT_TO_LEFT, stDisplayColor.text);
			break;
		...
		case MODE_COUNTDOWN :			
			...
			if(nPTimeList != 10)
			{
				dmd_draw_line(3, 2, 10, 2, stDisplayColor.counter);
				dmd_draw_line(3, 2, 3, 13, stDisplayColor.counter);
				dmd_draw_line(3, 13, 56, 13, stDisplayColor.counter);
				
				dmd_draw_line(148, 2, 156, 2, stDisplayColor.counter);
				dmd_draw_line(156, 2, 156, 13, stDisplayColor.counter);
				dmd_draw_line(102, 13, 156, 13, stDisplayColor.counter);
				
				utils_Concat("MEMASUKI WAKTU", " ", sPrayerTime[nPTimeList], &sMessageBuff);
				
				dmd_draw_string(dmd_get_textcenter(sMessageBuff), 0, sMessageBuff, stDisplayColor.counter);
			}
			else
			{
				dmd_draw_line(51, 2, 56, 2, stDisplayColor.counter);
				dmd_draw_line(51, 2, 51, 13, stDisplayColor.counter);
				dmd_draw_line(51, 13, 56, 13, stDisplayColor.counter);
				
				dmd_draw_line(102, 2, 107, 2, stDisplayColor.counter);
				dmd_draw_line(107, 2, 107, 13, stDisplayColor.counter);
				dmd_draw_line(102, 13, 107, 13, stDisplayColor.counter);
				
				dmd_draw_string(dmd_get_textcenter("IQAMAH"), 0, "IQAMAH", stDisplayColor.counter);
			}
			
			utils_Timestamp	((nPTimeCounter / 60), (nPTimeCounter % 60), NONE, bColonState, &sMessageTmp);
			utils_Concat("-", sMessageTmp, "", &sMessageBuff);
			dmd_draw_string(dmd_get_textcenter(&sMessageBuff), 9, sMessageBuff, stDisplayColor.counter);
