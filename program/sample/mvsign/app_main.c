...
void leddmd_init(void)
{
	...
	leddmd_get_powersave(&stPwrSave);
}

void leddmd_set_powersave(uint8_t StartHour, uint8_t StartMinute, uint16_t Duration)
{
...
void leddmd_get_powersave(stPowerSave *stPwrSv)
{
...
eTaskStatus leddmd_set_mode(eDisplayMode mode, char *message)
{
	/* Local Variables */
	uint8_t			index[10]			;
	int16_t			posX				= 0;
...
void leddmd_chk_event(eTimeEvent	*eEventRet)
{
...
void leddmd_get_message(uint8_t task, char *str, uint8_t *mode, uint8_t *delay, uint8_t *iteration)
{
	uint16_t	i	= 0;
